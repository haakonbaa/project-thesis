...
\begin{enumerate}
    \item Kinematic-level control as described in ...
    \item Dynamic-level control as described in .... The damping of ... is chosen
        as
    \begin{enumerate}
            \item pseuodo-inverse
            \item damped pseudo-inverse, $\lambda = 0.1$
            \item variable damped pseudo-inverse, $\lambda = 0.1$, $\epsilon = 0.1$
    \end{enumerate}
    \item Kinematic level controll with disturbance and erroneous model (gain matrices
    based on the model)
    \item Dynamic level control with disturbance and erroneous model
\end{enumerate}
\fi
